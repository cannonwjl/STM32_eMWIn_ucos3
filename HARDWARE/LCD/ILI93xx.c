...
{ 					
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_FSMC,ENABLE);	//使能FSMC时钟
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOE|RCC_APB2Periph_GPIOC,ENABLE);//使能PORTB,D,E,G以及AFIO复用功能时钟
	//---------------------------------------------------------------//
 //ILI9341_BK_CLK    
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;				 //PA11 推挽输出 背光
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOA, &GPIO_InitStructure);
	// GPIO_SetBits(GPIOA,GPIO_Pin_11);	
	
	   	//	//复用推挽输出 PE1  复位引脚 ILI9341_RST_CLK    换 PC11
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;	 //	//PORTD复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOC, &GPIO_InitStructure);
	
	  	//PD4 读使能     ILI9341_RD_CLK 
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;	 //	//PORTD复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOD, &GPIO_InitStructure);
	
		//PD5  写使能        ILI9341_WR_CLK 
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;	 //	//PORTD复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOD, &GPIO_InitStructure);
	
			//PD11     ILI9341_DC_CLK       这里使用 FSMCA16
			//PDE2     尝试使用      FSMCA23
			//DC引脚，使用FSMC的地址信号控制，本引脚决定了访问LCD时使用的地址
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;	 //	//PORTD复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOE, &GPIO_InitStructure);
	
				//PD7   //片选，选择NOR/SRAM块 ILI9341_CS_PIN	
  ...
  FSMC_NORSRAMInit(&FSMC_NORSRAMInitStructure);  //初始化FSMC配置
	...
	delay_ms(50); 
	GPIO_SetBits(GPIOC,GPIO_Pin_11);//GPIO_ReSet();	
