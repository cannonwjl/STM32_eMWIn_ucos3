...
//	printf("\n\nerror0=0x%02X\r\n",error);
	delay_ms(100); 
	error = SHT3X_GetTempAndHumi(&temperature, &humidity, REPEATAB_HIGH, MODE_POLLING, 50);	delay_ms(100);  
  delay_ms(100);
...
// error = SHT3X_GetTempAndHumi(&temperature, &humidity, REPEATAB_HIGH, MODE_POLLING, 50);	
 ...
 printf("\r\n SHT3X_StartPeriodicMeasurment=%d",error);
  if(error != NO_ERROR){printf("\r\n SHT3X_StartPeriodicMeasurment ok ");}      
//  RESET_HIGH();
}
...
etError SHT3x_ReadSerialNumber(u32t* serialNumber)
{
  ...
  error |= SHT3X_WriteCommand(CMD_READ_SERIALNBR);
	printf("\n\r SHT3X_WriteCommand  error=0x%02x",error);
  ...
  if(error == NO_ERROR)
  {
    *serialNumber = (serialNumWords[0] << 16) | serialNumWords[1];
		printf("\r\n serialNbr=%d \r\n ",(serialNumWords[0] << 16) | serialNumWords[1]);
  }
...
etError SHT3X_ReadMeasurementBuffer(ft* temperature, ft* humidity)
{
  ...
  if(error == NO_ERROR) error = SHT3X_StartReadAccess();  
  if(error == NO_ERROR) error = SHT3X_Read2BytesAndCrc(&rawValueTemp, ACK, 10);
  if(error == NO_ERROR) error = SHT3X_Read2BytesAndCrc(&rawValueHumi, NACK, 0);
...
////-----------------------------------------------------------------------------
//boolbit SHT3X_ReadAlert(void)
//{
//  // read alert pin
//  return (ALERT_READ != 0) ? TRUE1 : FALSE0;
//}
