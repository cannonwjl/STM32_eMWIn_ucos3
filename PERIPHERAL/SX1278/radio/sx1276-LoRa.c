...
#define LoRa_FREQENCY   438000000
uint32_t LoRa_FREQENCY_R=433000000;
u16 PreambleLength=8;
...
//static uint32_t TxPacketTimeout;
u8 cad_flag;
/*!
...
void SX1276LoRaInit( void )
{
    RFLRState = RFLR_STATE_IDLE;
		LoRaSettings.RFFrequency=LoRa_FREQENCY_R;
...
//#endif
    ...
    SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
		TxPacketTimeout=LoRaSettings.TxPacketTimeout;
}
...
void SX1276LoRaSetOpMode( uint8_t opMode )//设置工作模式
{
...
void SX1276_start_CAD()/***********  加入低功耗扫描的 **********/
{
    RFLRState = RFLR_STATE_CAD_INIT;
}

void SX1278_SLEEP_Mode()
{
	SX1276LoRaSetOpMode( RFLR_OPMODE_SLEEP );
}
void SX1278_STANDBY_Mode()
{
	SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
}
...
{
 SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
    SX1276Write( REG_LR_PREAMBLEMSB,200);
    SX1276Write( REG_LR_PREAMBLELSB,255);
    SX1276Write( REG_LR_PAYLOADLENGTH,LoRaSettings.PayloadLength);
    SX1276Write( REG_LR_IRQFLAGSMASK, ~(RFLR_IRQFLAGS_RXDONE_MASK|\
        RFLR_IRQFLAGS_PAYLOADCRCERROR_MASK)); 
    SX1276Write( REG_LR_DIOMAPPING1, RFLR_DIOMAPPING1_DIO0_00 );//Rx Done->DIO0
...
//			printf("RFLR_STATE_RX_INIT\n");
        SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
			...
			SX1276Write( REG_LR_PREAMBLEMSB,1);//set preamble length
			SX1276Write( REG_LR_PREAMBLELSB,255);//set preamble length
...
//			printf("RFLR_STATE_RX_DONE\n");
    ...
    case RFLR_STATE_TX_INIT:
		{
        SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
			SX1276Write( REG_LR_PREAMBLEMSB,(uint8_t)(PreambleLength>>8));//set preamble length
			SX1276Write( REG_LR_PREAMBLELSB,(uint8_t) PreambleLength);//set preamble length
    ...
    case RFLR_STATE_TX_RUNNING:
	{
        ...
        if( DIO2 == 1 ) // FHSS Changed Channel
        {
            ...
            SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_FHSSCHANGEDCHANNEL );
				...
				if( ( GET_TICK_COUNT( ) - TxTimeoutTimer ) > TxPacketTimeout )   //计算是否超时
				{
...
//			 printf("RFLR_STATE_CAD_RUNNING\n");
        ...
        { 
            ...
            {
							cad_flag=1;
							printf("CAD detected, we have a LoRa preamble\n");
                // Clear Irq
